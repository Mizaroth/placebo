		...
		if(update != null && update.hasMessage()) {
			...
			if(userFrom != null && ("Mizaroth".equals(userFrom.getUserName()) || "cbarbato".equals(userFrom.getUserName()) )&& update.getMessage().getChat() != null && update.getMessage().getChat().getTitle() == null) {
				if(update.getMessage().hasPhoto()) {
					...
					actionPerformed = TelegramApiWrapper.send(this, TelegramMessageTypeEnum.PHOTO, chatId, fileId, caption);
				}

				if(update.getMessage().getVoice() != null) {
					String fileId = update.getMessage().getVoice().getFileId();
					String caption = "File ID: " + update.getMessage().getVoice().getFileId();
					actionPerformed = TelegramApiWrapper.send(this, TelegramMessageTypeEnum.VOICE, chatId, fileId, caption);
				}
			}
			...
			if(RNGHandler.procByPercentage(10)) {
				...
				actionPerformed = TelegramApiWrapper.send(this, TelegramMessageTypeEnum.VOICE, chatId, fileId);
			}
			...
			if(message != null) {
				...
				if(RNGHandler.procByPercentage(1)) {
					...
					actionPerformed = TelegramApiWrapper.send(this, TelegramMessageTypeEnum.MESSAGE, chatId, text);
				}
